...
void test_inode_ialloc()
{
    ...
    printf("Output bit: %d\n",ialloc());
    bread(0, outbuff);
    CTEST_ASSERT(outbuff[0] == 1,  "Testing allocating inode and marking first inode as not free");
    printf("Output bit: %d\n",ialloc());
    bread(0, outbuff2);
    CTEST_ASSERT(outbuff2[1] == 0b0000001,  "Testing allocating inode and marking second inode as not free");
}
...
void test_inode_alloc()
{
    ...
    printf("Output bit: %d\n",alloc());
    bread(0, outbuff);
    CTEST_ASSERT(outbuff[0] == 1,  "Testing allocating block and marking first block as not free");
    printf("Output bit: %d\n",alloc());
    bread(0, outbuff2);
    CTEST_ASSERT(outbuff2[1] == 0b0000001,  "Testing allocating block and marking second block as not free");
}
